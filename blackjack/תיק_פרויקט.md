...
**מדוע בחרתי בפרויקט זה:** בלאק‑ג'ק פשוט מספיק בחוקיו כדי שהמיקוד יישאר בנושאי החלופה – תקשורת, ריבוי לקוחות, שמירת מצב והגנת סייבר – אך מורכב מספיק כדי לדרוש מכונת מצבים, סנכרון בין שחקנים ולוגיקה של קלפים.
...
הפתרון שלי שונה בכך שהוא **רב‑משתתפים אמיתי על TCP גולמי**, עם **לחיצת יד מוצפנת שכתובה ידנית** ושמירת פרופילים בקובץ מקומי – כך שכל שכבה ניתנת לקריאה ולהבנה.

### 1.1.6 סקירת טכנולוגיית הפרויקט

הפרויקט כולו ב‑**Python 3.11**. רוב הספריות סטנדרטיות (socket, threading, json, struct, secrets, hmac, hashlib, tkinter). ספריות חיצוניות: **cryptography** (RSA + Fernet) ו‑**google-genai** (Gemini AI לסינון צ'אט).
...
* שמירת נתונים בקובץ JSON עם כתיבה אטומית.
...
| איבוד קובץ פרופילים בכתיבה | כתיבה אטומית temp + os.replace | ממומש ב‑_save_locked() |
...
* **פעולות:** ולידציה, hash_password (PBKDF2), הוספה לפרופילים, שמירה אטומית.
...
* **מחשב שרת:** מריץ python -m blackjack.run_server. צריך Python 3.11, חיבור רשת, מקום לשני קבצים (profiles.json, server_rsa.pem).
...
| אחסון | קובץ JSON, כתיבה אטומית | פשוט, עמיד בפני קריסה |
...
### 2.8.1 קובץ profiles.json

| שדה | טיפוס | דוגמה |
|---|---|---|
| username | str (3-16 תווים, A-Za-z0-9_) | "alice" |
| password_hash | str (salt_b64$hash_b64) | "k4Ja…$g2Cv…" |
| credits | int >= 0 | 9750 |
| wins | int >= 0 | 4 |
| losses | int >= 0 | 2 |
| pushes | int >= 0 | 1 |

כתיבה אטומית: נכתב ל‑profiles.json.tmp ואז os.replace.
...
* ProfileManager.profiles : Dict[str, UserProfile]
...
* AppController._queue : queue.Queue
...
| גישה לנתוני משתמש אחר | כן | _require_auth; snapshot_for מסתיר קלף נעלם |
...
* **פעולות:** ולידציה; base_value; is_ace; to_dict/from_dict.

#### Deck (common/deck.py)
* **תפקיד:** "shoe" של N חבילות מעורבבות.
* **תכונות:** _cards: list[Card], num_decks: int.
* **פעולות:** reshuffle() – Fisher-Yates + secrets.randbelow; draw() → Card; __len__.
...
* **תפקיד:** טוען/שומר profiles.json; רישום, אימות, שינוי קרדיטים, עדכון תוצאות.
* **פעולות:** register(u,p); authenticate(u,p); adjust_credits; record_result; get.
...
* **תכונות:** _players, _dealer, _deck, _phase, _lock: RLock, callbacks.
* **פעולות:** add_player; place_bet; player_action; _start_round_locked; _run_dealer_locked; _resolve_locked; snapshot_for(username).

#### ClientHandler (server/client_handler.py) – thread
* **תפקיד:** thread אחד לכל לקוח.
* **פעולות:** run(); _handshake(); _main_loop(); _dispatch(); _handle_login/register(); send(); shutdown().

#### BlackjackServer (server/server.py)
* **תפקיד:** listening socket, accept loop, broadcasts, תזמון סבב.
* **פעולות:** serve_forever(); stop(); on_client_authenticated; on_client_disconnected; handle_place_bet; handle_action; handle_chat; _broadcast_state.

#### BlackjackClient (client/network.py)
* **תפקיד:** שכבת רשת בלקוח כולל thread קבלה.
* **פעולות:** connect(); _handshake(); send(); _recv_loop(); close().

#### AppController (client/gui.py)
* **תפקיד:** בקר ראשי – מחבר בין רשת ל‑GUI.
* **פעולות:** run(); send(); do_login; do_register; do_logout; _enqueue_message; _drain_messages; _handle_message; _show_login/_show_table.

#### ChatModerator (server/chat_moderator.py)
* **תפקיד:** סורק הודעות צ'אט עם Gemini.
* **פעולות:** check(message) → (allowed, reason).

#### ServerApp (server/gui.py)
* **תפקיד:** לוח בקרה גרפי לשרת.
* **פעולות:** run(); _poll() כל 400ms; _kick_player(); _on_stop(); _refresh_players().
...
|---|---|---|---|
| 1 | הרשמה תקינה | profiles.json קיבל רשומה; GUI עבר לשולחן | – |
...
| 11 | ניתוק באמצע סבב | תור עבר הלאה; הימור נחשב הפסד | נוסף טיפול ב‑on_client_disconnected |
...
├── README.md
├── תיק_פרויקט.md
├── flowchart.html             <- תרשים זרימה high-level
├── flowchart_deep.html        <- תרשים לכל קובץ ופונקציה
...
## נספח א' – פלואו צ'ארטים

* flowchart.html – מסביר את חוויית המשחק (13 שלבים).
* flowchart_deep.html – כל קובץ, מחלקה ופונקציה (~90 פריטים).

## נספח ב' – הסבר טכנולוגיות
