...
|---|---|---|---|
| 1 | הרשמה תקינה | רשומה נוצרה ב‑profiles.db; GUI עבר ישר לשולחן | – |
| 2 | רישום כפול של אותו שם | "username already taken" | – |
| 3 | סיסמה שגויה | "Invalid username or password" (גנרי, לא חושף אם המשתמש קיים) | בגרסה ראשונה חשפתי "unknown user"; תוקן |
| 4 | login כפול מאותו שחקן | "User already logged in"; החיבור הראשון לא הופרע | – |
| 5 | משחק מלא לשני שחקנים | היתרות עודכנו נכון; 3:2 על בלאק‑ג'ק | בהתחלה שילמתי 1:1 על בלאק‑ג'ק; תוקן |
| 6 | הימור מעל יתרה | "Insufficient credits"; יתרה לא השתנתה | – |
| 7 | פעולה מחוץ לתור | "Not your turn" | – |
| 8 | האזנה לתעבורה עם tcpdump | אחרי handshake – רעש בינארי בלבד, לא קריא | – |
| 9 | הודעה ענקית (200 KB) | חיבור נסגר אוטומטית, שרת המשיך לעבוד | recv_frame בודק את הגודל לפני הקצאת מאגר |
| 10 | הפעלה מחדש של השרת | קרדיטים וניצחונות נשמרו | – |
| 11 | ניתוק באמצע סבב | התור עבר אוטומטית הלאה; הימור נחשב הפסד | נוסף טיפול ב‑on_player_left |
| 12 | שם משתמש עם תווים מיוחדים | נדחה: "only letters, digits and underscore" | – |
...
| 14 | הרשאות קובץ PEM | -rw------- (0o600) | בהתחלה נשמר עם 644; נוסף os.chmod מיד אחרי השמירה |
| 15 | JSON לא תקין אחרי handshake | ProtocolError; החיבור נסגר | – |
| 16 | 10 לקוחות בו‑זמנית | כולם עברו handshake; CPU <5% | – |
| 17 | בדיקת אקראיות (secrets vs random) | אין דפוס ניתן לחיזוי | – |
| 18 | ניתוק רשת פתאומי | "Connection lost"; חזרה למסך ההתחברות | – |
